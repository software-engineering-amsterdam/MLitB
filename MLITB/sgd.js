...
SGDTrainer.prototype = {
  ...
  reduce : function(nn,param){
    ...
    console.log('after converting '+JSON.stringify(new_parameters.parameters[new_parameters.parameters.length-1]));

    step = nn.step;


    var totalError=new_parameters.error;
    var totalVector=new_parameters.nVector;
    ...
    // console.log(new_parameters.parameters);
    for (var i = 0; i < this.last_params.length; i++) {
      ...
      for (var gi = 0, glen=g.length; gi < glen; gi++) {
        var j = g[gi][0];
        var gval = g[gi][1];
        ...
        ssg[j] += ((gval/totalVector) *(gval/totalVector));
        ...
        var dw = this.learning_rate/tess*((l1_grad+l2_grad+gval)/totalVector);
        p[j] -= dw; //x = x - gradient
    ...
    console.log('last param '+JSON.stringify(this.last_params[this.last_params.length-1]));
